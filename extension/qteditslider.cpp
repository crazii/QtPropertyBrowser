...
/* line edit + slider                                                                     */
/************************************************************************/
class QtEditSlider : public QWidget
{
public:
	Q_OBJECT
public:
	QtEditSlider(QWidget* parent = Q_NULLPTR)
		:QWidget(parent)
		,mEdit(this)
		,mSlider(this)
	{
...
/************************************************************************/
/* text(readonly) + slider for enums                                                                    */
/************************************************************************/
class QtEnumSlider : public QWidget
{
public:
	Q_OBJECT
public:
	QtEnumSlider(QWidget* parent = Q_NULLPTR)
		:QWidget(parent)
		,mEdit(this)
		,mSlider(this)
	{
	...
	{
		//TODO:
        Q_UNUSED(index);Q_UNUSED(icon);
	}

	private slots:
		void sliderMoved(int pos)
		{
			assert(pos < mEnums.count());
...
/*                                                                      */
/************************************************************************/
class QtDoubleEditSliderFactoryPrivate : public EditorFactoryPrivate<QtEditSlider>
{
...
{
	QList<QtEditSlider *> editors = m_createdEditors[property];
	QListIterator<QtEditSlider *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QtEditSlider *editor = itEditor.next();
													 ...
													 double min, double max)
{
	...
	QList<QtEditSlider *> editors = m_createdEditors[property];
	QListIterator<QtEditSlider *> itEditor(editors);
	while (itEditor.hasNext()) {
		QtEditSlider *editor = itEditor.next();
		editor->blockSignals(true);
...
void QtDoubleEditSliderFactoryPrivate::slotSetValue(double value)
{
	...
	const QMap<QtEditSlider *, QtProperty *>::ConstIterator itcend = m_editorToProperty.constEnd();
	for (QMap<QtEditSlider *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != itcend; ++itEditor) {
		if (itEditor.key() == object) {
			QtProperty *property = itEditor.value();
			QtDoublePropertyManager *manager = q_ptr->propertyManager(property);
			if (!manager)
...
{
	d_ptr = new QtDoubleEditSliderFactoryPrivate();
	d_ptr->q_ptr = this;
}
...
{
	QtEditSlider *editor = d_ptr->createEditor(property, parent);
...
/*                                                                      */
/************************************************************************/
class QtIntEditSliderFactoryPrivate : public EditorFactoryPrivate<QtEditSlider>
{
...
void QtIntEditSliderFactoryPrivate::slotSetValue(int value)
{
	QObject *object = q_ptr->sender();
	const QMap<QtEditSlider *, QtProperty *>::ConstIterator itcend = m_editorToProperty.constEnd();
	for (QMap<QtEditSlider *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != itcend; ++itEditor) {
...
{
	d_ptr = new QtIntEditSliderFactoryPrivate();
	d_ptr->q_ptr = this;
}
...
/*                                                                      */
/************************************************************************/
class QtEnumSliderFactoryPrivate : public EditorFactoryPrivate<QtEnumSlider>
{
...
void QtEnumSliderFactoryPrivate::slotPropertyChanged(QtProperty *property, int value)
{
	...
	QListIterator<QtEnumSlider *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QtEnumSlider *editor = itEditor.next();
		editor->blockSignals(true);
...
{
	QObject *object = q_ptr->sender();
	const  QMap<QtEnumSlider *, QtProperty *>::ConstIterator ecend = m_editorToProperty.constEnd();
	for (QMap<QtEnumSlider *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
		if (itEditor.key() == object) {
			QtProperty *property = itEditor.value();
			QtEnumPropertyManager *manager = q_ptr->propertyManager(property);
			if (!manager)
...
{
	d_ptr = new QtEnumSliderFactoryPrivate();
	d_ptr->q_ptr = this;
...
QWidget *QtEnumSliderFactory::createEditor(QtEnumPropertyManager *manager, QtProperty *property,
										   QWidget *parent)
{
	QtEnumSlider *editor = d_ptr->createEditor(property, parent);
	//editor->setSizeAdjustPolicy(QtEnumSlider::AdjustToMinimumContentsLengthWithIcon);
...
#include "moc_qteditslider.cpp"
#include "qteditslider.moc"
